...
workflow build type) -> M30 (two documented procedures that mislead) and M31
(two code defects found beside them), both opened 2026-08-07 from the document
review and both required before the M7 gate begins -> then the M7 release gate.
...
| M30 Misleading documented procedures | Two documented procedures that a reader can follow into a silently wrong result (#57) | Milestone | Not started | Found by the 2026-08-07 document review; owner directed that the code stays and the documents move to match it. (a) `README.md` Base Commands presents `make patch.base` as applying the needed patches, but `configure/RULES_BASE:8` binds it to `patch.base.apply` alone and never reaches `patch.base.pr.apply` (`configure/RULES_PATCH:21`), the leg that applies the fifteen M22 carry patches; the top-level `patch:` (`configure/RULES_SRC:14`) calls both. Measured: `make -n patch.base` emits no PR-carry line, `make -n patch` does. (b) Two guides point at `MODS_ZERO_VARS`, which `configure/RULES_MODS_CONFIG:48` derives from `$(MOD_CONF_AUTO_TARGETS) $(MODS_ZERO_CUSTOM_VARS)`; the hand-maintained list is `MODS_ZERO_CUSTOM_VARS` at line 47. They fail differently and both were measured on a scratch clone. `remove-a-module.md:40` says without condition to delete a module's `conf.*` entry from `MODS_ZERO_VARS`; line 48 holds no such string, so the edit changes nothing — the target count stays at 16 and the module's configure target is still there, while the reader believes it was removed. `new-module-example.md:40-43` is a fork: base-only dependencies go to `MODS_ZERO_VARS`, multi-module ones to `MODS_ONE_VARS`. Its pmac example takes the second branch and edits the literal list at line 57, so the example itself is correct; a reader taking the first branch and expanding `MODS_ZERO_VARS` into a literal drops it from 16 targets to 8 and removes the configure target of all nine auto modules |
| M30.T1 | `README.md` states what `make patch.base` really does, and points to the target that applies the carry set | Verification | Not started | |
| M30.T2 | Both guides name `MODS_ZERO_CUSTOM_VARS` as the edit point for a custom module. On a scratch clone: following the add path leaves all nine auto configure targets intact, and following the remove path actually drops the retired module's target from the derived list | Verification | Not started | |
| M31 Patch-helper guard and open62541 spelling | Two code defects surfaced beside the document review (#58) | Milestone | Not started | Recorded 2026-08-07; code-side, so held out of the document pass and carried here instead. (a) Six helpers in `configure/RULES_FUNC` loop `patch` over a wildcard list — the direct `patch` calls in `configure/RULES_PATCH` for mca, opcua, opcua.export, and feed-core are recipe lines, where make already stops on failure, and the pvxs block beside them is commented-out history; none is counted here. Four of the six carry `\|\| exit 1` and two do not. Unguarded: `base_patch_src` (line 7) and `base_revert_patch_src`, present since the repository's first commit `35d9a9f` (2020-07-23) and only renamed by `6623eae` from `patch_src` / `revert_patch_src`. Guarded: `base_pr_patch_src` and `base_pr_revert_patch_src` from `c7aac56` (2026-07-25, M22), and `pvxs_commit_patch_src` and `pvxs_commit_revert_patch_src` from `c3a42c3` (2026-07-26, M26). Neither carry wiring revisited the original pair, so the asymmetry is an accident of sequence, not a designed difference. The wildcard matches no file at base 7.0.10 so the loop runs zero times today, but a `7.0.11.base.p0.patch` added at the next bump would make a failing apply silent. (b) `tools/prep-vendors.bash` carries three spellings of open62541 across six lines. The help text prints `prep-open62451` and "Prepare open62451" on line 316 and `open65451` on line 317, while the dispatcher accepts only `prep-open62541`, so either spelling typed from the help returns `Error: Unknown command`. The variable `VENDOR_OPEN62451_SRC` at lines 57, 179, and 214 transposes the same two digits; its value resolves to the correct `open62541-env` path and all three uses agree, so nothing breaks, but line 214 pairs the correctly named function `prep_open62541` with the misspelled variable. The uldaq sibling at lines 56, 178, and 213 shows the intended shape |
| M31.T1 | All six patch helpers — three apply legs and three revert legs — stop the target when their `patch` invocation fails; a deliberately failing patch file proves it on the version-specific base leg | Verification | Not started | |
| M31.T2 | Every command name in the `prep-vendors.bash` help text is accepted by its dispatcher, no `62451` or `65451` spelling remains in the file, and `prep-open62541` still resolves its vendor source path | Verification | Not started | |
...
Tally: Milestones 29 (Complete 26, Not started 3 — M30, M31, and the M7 gate) · Verification subs 40 (Complete 32, Not started 8)
