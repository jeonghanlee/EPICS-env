...
verified; sticky repo-URL fixed in check and update) -> M33 Base carry refresh
against the current `7.0` branch, M34 asyn R4-46 adoption, M23 CI vendor
relocation (returned from 1.3.1 on 2026-08-17), and M36 mdbook image adoption
in any order, all Complete -> the M7 release gate. The 1.3.0 target remains the
...
| M36 mdbook image adoption | Move the docs build from a direct mdbook install to the shared `jeonghanlee/mdbook` image (register-local, no tracker issue) | Milestone | Not started | Today `.github/workflows/docs.yml` installs mdbook `v0.5.4` by curl (`MDBOOK_VERSION`) into the Pages runner and runs `mdbook build docs`. The fleet now ships `jeonghanlee/mdbook` at mdbook 0.5.4 on Debian 13 trixie with the PDF tool set and an owner-gated `workflow_dispatch` publish (Dockerfiles `df6d71b`, `jeonghanlee/mdbook:latest` 2026-08-17); adopt it so the docs build tracks the shared image instead of a pinned download. `docs/book.toml` has no `git-repository-icon` line, so the mdbook 0.5 Font Awesome 6 icon-prefix break does not apply here. Owner sequencing against M7 and the work order is open |
| M36.T1 | The docs site renders through the `jeonghanlee/mdbook` image with output equivalent to the current direct-install build | Verification | Not started | Build `docs` in the `jeonghanlee/mdbook` container and compare the rendered site to the current 0.5.4 direct-install result; page set and structure unchanged |

Tally: Milestones 35 (Complete 30, Not started 5: M23, M33, M34, M36, and M7); Verification subs 44 (Complete 38, Not started 6); Release Verification 10 (Pending 10)
