...
2026-08-08, T1 and T2 verified -> M32 upstream survey integrity DONE
2026-08-17 (`1ee3cae`, T1/T2 verified; Option 2 transport-distinguishing exit
codes 0/1/2) -> M35 survey repo-URL binding DONE 2026-08-17 (`f64f8a3`, T1
verified; sticky repo-URL fixed in check and update) -> M36 mdbook image
adoption DONE 2026-08-17 (`bee8bbb`, T1 verified; docs build via container
`jeonghanlee/mdbook`) -> M33 Base carry refresh against the current `7.0`
branch, M34 asyn R4-46 adoption, and M23 CI vendor relocation (returned from
1.3.1 on 2026-08-17) in any order, all Complete -> the M7 release gate. The 1.3.0 target remains the
...
| M32 Upstream survey integrity | Make `tools/update-release.bash check` distinguish a complete survey from remote lookup failure (register-local, no tracker issue) | Milestone | Complete | Original problem: on 2026-08-11 the shipped command printed `All modules are up to date.` after every `git ls-remote` lookup failed (a lookup failure left `updates_found=0`, a false green). Owner decision 2026-08-17: distinguish the transport boundary (Option 2) rather than fold every failure into one code. Landed `1ee3cae`: `_check_updates_only` counts every matched pin and partitions it into surveyed / unreachable / no-URL / indeterminate; `_remote_head_status` and `_latest_release_tag` capture the `git ls-remote` result before parsing and return 2 (transport failure) vs 1 (reachable, no release tag / HEAD). Exit-code contract: 1 incomplete survey (a pin unreachable OR without a repository URL, results not authoritative), 2 completed but a pin indeterminate, 0 complete and authoritative; the up-to-date message is unreachable when any transport/unresolved failure exists. `usage` documents the codes. First-, second-, and third-person review, 0 blocking. A sibling silent-skip surfaced in review (sticky `current_repo_url` for a mid-file missing URL comment) is out of M32 scope and parked as M35 |
| M32.T1 | A successful survey through the shipped command completes every configured lookup and reports available updates, including asyn R4-46 | Verification | Complete | 2026-08-17T12:29-0700 on this host, real `check` against the official remotes with no substitution: 30/30 pins surveyed, 9 updates reported including ASYN R4-45 -> R4-46, EXIT 0 |
| M32.T2 | A controlled failure at the outermost Git transport boundary returns nonzero, identifies the failed lookup, and does not print the up-to-date result | Verification | Complete | 2026-08-17 real `check` with only `git ls-remote` forced to fail (PATH shim over the shipped RELEASE and real aggregation): 30/30 reported LOOKUP FAILED by name, incomplete-survey line to stderr, EXIT 1, "up to date" absent (grep 0). Two adjacent paths exercised on the real parse path: a missing repository URL -> UNRESOLVED, EXIT 1; a reachable remote with only a filtered non-release tag -> INDETERMINATE, EXIT 2 |
| M33 Base carry refresh | Re-evaluate and regenerate the R7.0.10 carry set against the current upstream `7.0` branch (GitHub #60) | Milestone | Not started | Keep Base pinned at the latest full release, R7.0.10. Re-run the complete `R7.0.10...7.0` enumeration under `docs/upstream-fix-carry-procedure.md`, reconcile the existing carry with every newer security, correctness, and compatibility change, record the owner selection, and regenerate every selected patch whose upstream context moved. New upstream changes remain in scope until release; there is no fixed cutoff |
...
| M34 asyn R4-46 adoption | Move the asyn pin from R4-45 to the released R4-46 tag and reconcile local compatibility work (GitHub #61) | Milestone | Not started | Review the R4-45 to R4-46 change set, update the release pin and version, and determine from a real Ubuntu 26 build whether asyn can leave `MODS_C17_SRC_PATHS`. Check every module and site consumer that links to or configures asyn; do not treat a successful source build alone as completion |
...
| M35 Survey repo-URL binding | `_check_updates_only` and `_process_release_file` bound each pin to the last-seen `# https://` comment, which persisted across module blocks (GitHub #62) | Milestone | Complete | Found in the M32 review 2026-08-17. `current_repo_url` was sticky: a module whose own URL comment is missing or malformed inherited the previous module's URL and was surveyed against the wrong repository -- `check` reported the wrong repo, `update` could write a wrong "latest" into RELEASE. Landed `f64f8a3`: both functions bind a `pending_repo_url` at each `SRC_NAME` and clear it, so a module without its own URL reports UNRESOLVED (check: exit 1; update: pin kept unchanged, not surveyed). Pattern audit expanded scope from check-only to both paths. Boundaries recorded: (a) update's UNRESOLVED is a per-line notice, not summarized in `_finalize_changes`; (b) the fix encodes one URL comment per module, so a future intentional shared-URL (monorepo) layout would report UNRESOLVED. Plan and both code changes passed first-, second-, and third-person review, 0 blocking |
| M35.T1 | A module block with no URL comment reports UNRESOLVED rather than being surveyed against a sibling remote; the normal per-module binding is unchanged | Verification | Complete | 2026-08-17 on the real shipped functions (only the git transport shimmed): mid-file AAA(own URL) -> BBB(no URL) -> CCC(own URL) -- pre-`f64f8a3` surveyed BBB against AAA's remote; post-fix `check` reports BBB UNRESOLVED EXIT 1 and `update` reports BBB UNRESOLVED with the pin kept unchanged (the survey tripwire never fired for BBB), AAA/CCC bound to their own URLs. Real-RELEASE regression: `check` 0 UNRESOLVED, 30/30 surveyed, EXIT 0; `update` 0 UNRESOLVED, "No updates applied", RELEASE byte-identical. Leading no-URL case still UNRESOLVED. `bash -n` / `shellcheck` clean |
...
| M36 mdbook image adoption | Move the docs build from a direct mdbook install to the shared `jeonghanlee/mdbook` image (register-local, no tracker issue) | Milestone | Complete | Adopted 2026-08-17 (`c485b65` + `bee8bbb`): the `.github/workflows/docs.yml` build job runs in `container: jeonghanlee/mdbook` (the published `:latest`, mdbook 0.5.4 on Debian 13 trixie), replacing the curl install of `MDBOOK_VERSION v0.5.4`, matching the epics-trainings pattern. The lychee link check is dropped from this workflow by owner decision (link-check reproducibility is tracked in 1.3.1 #59). A first attempt pinned the local-only `:0.5.4-test` tag and failed to pull on the Pages runner; `bee8bbb` switched to the published `jeonghanlee/mdbook`. `docs/book.toml` has no `git-repository-icon` line, so the mdbook 0.5 Font Awesome 6 icon-prefix break does not apply |
| M36.T1 | The docs site renders through the `jeonghanlee/mdbook` image with output equivalent to the current direct-install build | Verification | Complete | 2026-08-17: a container build of `docs` with `jeonghanlee/mdbook` (`:latest`, mdbook 0.5.4) is byte-identical to a direct mdbook v0.5.4 build (`diff -r` clean over the full book); the in-container `git status --porcelain -- docs/src` guard passes; YAML valid. CI run 32101260400 on `bee8bbb` is green -- Build the mdBook site and Deploy to GitHub Pages both succeed, github-pages artifact published |

Tally: Milestones 35 (Complete 31, Not started 4: M23, M33, M34, and M7); Verification subs 44 (Complete 39, Not started 5); Release Verification 10 (Pending 10)
...
Implementation Authorization: M32 authorized and implemented 2026-08-17
(`1ee3cae`); pending for M33-M35. The owner authorized the review corrections
on 2026-08-11; that authorization does not cover code, commits, GitHub
mutations, or release actions for the remaining register-local milestones.
...
#61, and #62. Reconcile again before tracker closure.
