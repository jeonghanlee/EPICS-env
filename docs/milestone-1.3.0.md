...
codes 0/1/2) -> M33 Base carry refresh against the current `7.0` branch, M34
asyn R4-46 adoption, M35 survey repo-URL binding, and M23 CI vendor relocation
(returned from 1.3.1 on 2026-08-17) in any order, all Complete -> the M7
release gate. The 1.3.0 target remains the
...
| M35 Survey repo-URL binding | `_check_updates_only` binds each pin to the last-seen `# https://` comment, which persists across module blocks (GitHub #62) | Milestone | Not started | Found in the M32 review 2026-08-17. `current_repo_url` is sticky: a module whose own URL comment is missing or malformed inherits the previous module's URL and is surveyed against the wrong repository instead of being flagged. The M32 UNRESOLVED bucket catches only a pin appearing before any URL comment (empty `current_repo_url`), not the mid-file inherited-URL case. Reset the bound URL at each `SRC_NAME_` block so a module without its own URL comment reports UNRESOLVED rather than silently surveying a sibling's remote. Clean up alongside the other 1.3.0 prep (owner 2026-08-17) |
| M35.T1 | A module block with no URL comment reports UNRESOLVED rather than being surveyed against a sibling remote; the normal per-module binding is unchanged | Verification | Not started | On a crafted RELEASE where a mid-file module omits its URL comment, confirm it is reported UNRESOLVED with EXIT 1, and that a well-formed RELEASE still surveys every pin against its own URL; exercise the real parse and aggregation path |
...
Tally: Milestones 35 (Complete 29, Not started 6: M23, M33, M34, M35, M36, and M7); Verification subs 44 (Complete 37, Not started 7); Release Verification 10 (Pending 10)
