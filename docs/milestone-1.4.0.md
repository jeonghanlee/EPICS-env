...
Next session entry point: RELEASED 2026-09-24 - EPICS-env 1.4.0 published (merge `5326c98`, annotated tag `1.4.0` object `f6c1c81`, GitHub release `396321308`, milestone 6 closed); Release Verification 1-10 Pass; M11 and every assigned milestone are Complete. The next release line is not open (D27): when its first work is assigned, choose 1.4.1 for fixes only or 1.5.0 for module-set or feature changes, then follow M11's Next Cycle Handoff (create `release-X.Y.Z` from master, reset this register into `docs/milestone-X.Y.Z.md`, set `ENV_RELEASE_VERS`). Backlog: M5 (#25), M8 (#75), M10 (#76), M13, M14. D12-D27 govern the accepted direction.
...
| 14 | release-1.2.0 local and remote deletion, if present and fully merged | User-run | Deletion or observed absence recorded | Observed absent on 2026-09-25: no local or remote `release-1.2.0`; nothing deleted (remaining release branches: local `release-1.3.0` and `release-1.4.0`; remote `release-1.2.2`, `release-1.3.0`, and `release-1.4.0`) |
| 15 | Closure preparation on master | Commit/add scope | Committed 1.4.0 evidence and D27 next-line deferral; M11 still In progress | `c5f8e7a`, `1adec43`, `d5c0924`, and `767751e`, pushed to origin/master at `767751e`; the two Release Verification 7 corrections `98182b9` and `8c1a87b`, pushed at `8c1a87b`; M11 In progress in each |
| 16 | Final checked 1.4.0 closure on master | Commit/add scope | All final checks Pass; M11 Complete; committed bytes match checked file | This closure commit; find it with `git log -S'RELEASED 2026-09-24 - EPICS-env' -- docs/milestone-1.4.0.md`, the only commit that adds that text |
| 17 | Final master push | New Push scope | Remote tip equals closure commit; no closure-path changes | The push of this closure commit; its receipt is reported outside this document |
...
| Release Verification 7 | 2026-09-25T17:11Z (UTC) | Committed register at `8c1a87b` on master | Pass | Release Verification 1-6 and 8-10 Pass; Release Execution rows 1-14 observed and row 15 carried by `c5f8e7a` through `8c1a87b`; every Release Execution row has five cells and markdownlint reports no broken detail link or table column error; the 11 linked issues match their projections (#56, #59, #68, #71, #72, #73, #74, and #77 closed in milestone 1.4.0; #25, #75, and #76 open in Backlog) and PR #70 is merged; M1-M4, M6, M7, M9, and M12 are Complete; D27 is recorded and the next entry point follows it. The first run on `767751e` found row 14 missing its expected-result cell and the M6 name contradicting D26; `98182b9` and `8c1a87b` corrected both before this result. |
...
- Plan: accepted and authorized 2026-09-24; revised 2026-09-25 for D27 and the PR #70 merge, accepted and authorized 2026-09-25.
- Tested source `fea9b7342ca801907c011bf9b8d0b285ed47c45b`; readiness candidate `deb487e3d21ec652ffce687502dc50a1e612d61c`; final candidate `f19c5234d4713810b6409132710037e2462070f3`.
- Merge `5326c981912566810f763cbe12aba6509bbdb7c4`; annotated tag `1.4.0` object `f6c1c81a0df390d63c2d0881a9617342933bbcbc` peeling to that merge; GitHub release `396321308`, https://github.com/jeonghanlee/EPICS-env/releases/tag/1.4.0, published 2026-09-25T05:10:01Z (UTC), Latest.
- Post-release installation: Release Verification 6 Pass on a fresh Debian 13 VM (run `rv6-20260925b`).
- Issues: #77 closed as completed; milestone 6 closed with 0 open and 9 closed items; PR #70 merged by the master push.
- Next line: not opened, per D27 (2026-09-25).
- Actual publication date: RELEASED 2026-09-24 (release timezone -0700).
