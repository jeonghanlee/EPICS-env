...
M4 rebuild. Per-OS verification: rocky8 + debian13 CONFIRMED clean (check_deps exit 0,
vendors in tree, via skill-only agent runs); rocky10 pending; version not yet bumped
...
| M1.T1 | `readelf.base` (both tags) + `readelf.modules`: zero DT_RPATH + DT_RUNPATH present, Rocky 8.10/10.2 (Debian unchanged); site-modules verified in the alsu repo | Verification | In progress | rocky8 + debian13 CONFIRMED via fresh skill-only VM builds (zero DT_RPATH, `libCom.so` RUNPATH `[$ORIGIN/.]`); Rocky 10.2 pending |
...
| M2.T1 | `check_deps.bash` (strict default) exits 2 on a populated RPATH tree, exits 0 on the corrected tree; `--report-only` exits 0; broadened `find` selects real `*.so.N`; empty-`$ORIGIN` exempts system-only blob | Verification | In progress | exit-2 / find 1->13 / `--report-only` / libVimbaC-exempt verified on real tree; exit-0 CONFIRMED on fresh rocky8 AND debian13 builds (RPATH 0, ABSPATH 0, check_deps exit 0; vendors installed into the tree's vendor/ via the role's install.bash method), rocky10 pending; lost-`$ORIGIN` FLAG has no natural fixture, constructed-object only |
