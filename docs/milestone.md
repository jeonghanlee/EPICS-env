...
| 5 | Deferred code changes (2) — RESOLVED, neither ships | (a) strict flip tried (38bfb59) and reverted: CI vendors at /usr/local put ABSPATH on measComp/opcua, so CI keeps report-only `audit.deps`; relocation is a 1.3.0 item · (b) no code needed: single script copy in EPICS-env, install.bash caller gated since 12777fa, strict arrives via the 1.2.2 checkout | (a) closes M5 (#50, criterion revised) · (b) closes M2 (#45, resolved by structure) |
